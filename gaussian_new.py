...
class GaussianTransformer(object):
    ...
    def __init__(self, imgSize=512, region_threshold=0.4,
        ...
        self.standardGaussianHeat_aff = self._gen_gaussian_heatmap_aff(imgSize, distanceRatio)
        ...
        _, binary = cv2.threshold(self.standardGaussianHeat_aff, affinity_threshold * 255, 255, 0)
        np_contours = np.roll(np.array(np.where(binary != 0)), 1, axis=0).transpose().reshape(-1, 2)
        x, y, w, h = cv2.boundingRect(np_contours)
    ...
    def _gen_gaussian_heatmap(self, imgSize, distanceRatio):
        ...
        heat[heat<255*0.1] = 255*0.1
        return heat

    def _gen_gaussian_heatmap_aff(self, imgSize, distanceRatio):
        scaledGaussian = lambda x: exp(-(1 / 2) * (x ** 2))
        heat = np.zeros((imgSize, imgSize), np.uint8)
        for i in range(imgSize):
            for j in range(imgSize):
    ...
    def add_region_character(self, image, target_bbox, regionbox=None, affinity=False):
, affinity=False
        ...
        if np.any(target_bbox[0] < real_target_box[0]) or (
            ...
            if affinity is False:
                ...
                warped = cv2.warpPerspective(self.standardGaussianHeat_aff.copy(), M, (image.shape[1], image.shape[0]))
            warped = np.array(warped, np.uint8)
            image = np.where(warped > image, warped, image)
    ...
    def add_affinity_character(self, image, target_bbox):
        return self.add_region_character(image, target_bbox, self.affinitybox, affinity=True)
